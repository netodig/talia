...
var SqueezeBox = {
	...
	fromElement: function(el, options) {
		...
		for (var key in this.parsers) {
			...
			if ((res = this.parsers[key].call(this))) 
			{
			return this.setContent(key, res);
			}
		}
		return this;
	},
	...
	resize: function(size, instantly) {
		...
		var to = {
			...
			marginTop: - this.size.y / 2 +this.options.reduccion
			//left: (sizes.scroll.x + (sizes.size.x - this.size.x - this.options.marginInner.x) / 2).toInt(),
			//top: (sizes.scroll.y + (sizes.size.y - this.size.y - this.options.marginInner.y) / 2).toInt()+this.options.reduccion
		};
	...
	handlers: {
		...
		'url': function(url) {
			aj= new AjaxNpg();
			aj.sendRequest(url,this.options.ajaxOptions.params,function(resp) {													
				this.applyContent(resp);

				if(this.options.ajaxOptions.onComplete)
				this.options.ajaxOptions.onComplete();
				this.ajax = null;
			}.bind(this),this.options.ajaxOptions.evalScripts);
			
			
			//include_page_down(url,'','sbox-content');
			/*this.ajax = new Ajax(this.url, this.options.ajaxOptions);
