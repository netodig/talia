...
g_l["MONTH_FWD"] = "Mes adelante";
g_l["MONTH_BCK"] = "Mes atrás";
g_l["YEAR_FWD"] = "Año adelante";
g_l["YEAR_BCK"] = "Año atrás";
g_l["CLOSE"] = "Cerrar";
...
/*
	...
	Thank you!
*/
g_jsDatePickImagePath = SITEURLCOMPLETA+"img/imgcal/";
...
g_currentDateObjectmasmeses = {};
g_currentDateObjectmasmeses.dateObject = new Date();
g_currentDateObjectmasmeses.day = g_currentDateObjectmasmeses.dateObject.getDate();
//g_currentDateObjectmasmeses.dateObject.setMonth(g_currentDateObjectmasmeses.dateObject.getMonth()+CANTMESES);
g_currentDateObjectmasmeses.month = g_currentDateObjectmasmeses.dateObject.getMonth() + 1;
g_currentDateObjectmasmeses.year = g_currentDateObjectmasmeses.dateObject.getFullYear();
...
g_currentDateObjectmasmeses.dateObject = new Date();
g_currentDateObjectmasmeses.day = g_currentDateObjectmasmeses.dateObject.getDate();
g_currentDateObjectmasmeses.dateObject.setMonth(g_currentDateObjectmasmeses.dateObject.getMonth()+configurationObject["cantmeses"]);
g_currentDateObjectmasmeses.month = g_currentDateObjectmasmeses.dateObject.getMonth() + 1;
g_currentDateObjectmasmeses.year = g_currentDateObjectmasmeses.dateObject.getFullYear();
	this.oCurrentDaymasmes = g_currentDateObjectmasmeses;
...
JsDatePick.prototype.setConfiguration = function(aConf){
	...
	this.oConfiguration.SegundoCalendar   	= (aConf["SegundoCalendar"] != null) ? aConf["SegundoCalendar"] : false;
	this.oConfiguration.usounidocalendar   	= (aConf["usounidocalendar"] != null) ? aConf["usounidocalendar"] : false;
	this.oConfiguration.funcionatclose= (aConf["funcionatclose"] != null) ? aConf["funcionatclose"] : false;
...
JsDatePick.prototype.populateFieldWithSelectedDate = function(){
	...
	if(!this.oConfiguration.SegundoCalendar && this.oConfiguration.usounidocalendar)
	{
		
		g('fechafin').value="";
		rehace();
	}
	if(this.oConfiguration.funcionatclose)
	this.oConfiguration.funcionatclose();
...
JsDatePick.prototype.isAvailable = function(y, m, d){
	...
	//chequeo si sirve para los meses despues
	if (y > this.oCurrentDaymasmes.year){
		return false;
	}
	
	if (m > this.oCurrentDaymasmes.month && y == this.oCurrentDaymasmes.year){
		return false;
	}
	
	
	//chequeo la seleccion del primer calendario
	if(this.oConfiguration.SegundoCalendar &&  this.oConfiguration.usounidocalendar)
	{
		//tomo la fecha
		fe=gv('fechaini').split("/");
		
		if (y < parseInt(fe[2])){
		return false;
	}
	
	if (m < parseInt(fe[1]) && y ==parseInt(fe[2])){
		return false;
	}
	
	if (d < parseInt(fe[0]) && m == parseInt(fe[1]) && y == parseInt(fe[2]) ){
		return false;
	}
