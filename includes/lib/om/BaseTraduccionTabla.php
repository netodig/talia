<?php
    class TraduccionTabla
    {
       ...
       public function TraduccionTabla()
       {
          $this->conn = JFactory::getDbo();          
          $this->Field = array(); 
          //aqui se van a guardar los formatos de los campos
          $this->formatFields = array('tabla'=>"`traduccion_tabla`.`tabla` = '%s'", 'campo'=>"`traduccion_tabla`.`campo` = '%s'", 'tipotrad'=>"`traduccion_tabla`.`tipotrad` = %s", 'tituloc'=>"`traduccion_tabla`.`tituloc` = '%s'", 'muestraentitulo'=>"`traduccion_tabla`.`muestraentitulo` = %s");
          //listado de llaves primarias
          $this->listPk = array();
          //formato de las llaves primarias
          $this->listPkFormat = array("id"=>"`traduccion_tabla`.`id` = %s");
          //listado de campos que contiene la tabla TraduccionTabla
          $this->listField = array("id"=>"id", "tabla"=>"tabla", "campo"=>"campo", "tipotrad"=>"tipotrad", "tituloc"=>"tituloc", "muestraentitulo"=>"muestraentitulo");
          ...
          $this->Field["tabla"] = "";
          $this->Field["campo"] = "";
          $this->Field["tipotrad"] = 0;
          $this->Field["tituloc"] = "";
          $this->Field["muestraentitulo"] = 0;		  
       }
       ...
       {
          return $query = sprintf("insert into `traduccion_tabla` (`traduccion_tabla`.`tabla`, `traduccion_tabla`.`campo`, `traduccion_tabla`.`tipotrad`, `traduccion_tabla`.`tituloc`, `traduccion_tabla`.`muestraentitulo`) VALUES('%s', '%s', %s, '%s', %s)", $this->Field["tabla"], $this->Field["campo"], $this->Field["tipotrad"], $this->Field["tituloc"], $this->Field["muestraentitulo"]);
       }
       ...
       {  
          //obtiene los campos a actualizar  
       	  ...
       	  return $query = "update `traduccion_tabla` set ".implode(", ", $intersecUpdateField)." where ".implode(" and ", $intersecPk);
       }
       
       public function Delete()
       {
          $query =sprintf("delete from `traduccion_tabla` where `traduccion_tabla`.`id` = %s", $this->Field["id"]);
       ...
       {
          if($query == "")          
            $query = "select * from `traduccion_tabla`";             
          
          return $this->Hydrating($query);          
       }
       
       /**
       * Este metodo devuelve un array con el objeto de tipo TraduccionTabla
       * dado un ID determinado
       */
       public function getTraduccionTablaById($Id)
       {
         return $this->SelectConditional(sprintf("where `traduccion_tabla`.`id` = %s", $Id));
       }
       ...
       */
       ...
       {
         $query = "select SQL_CALC_FOUND_ROWS `traduccion_tabla`.`id`, `traduccion_tabla`.`tabla`, `traduccion_tabla`.`campo`, `traduccion_tabla`.`tipotrad`, `traduccion_tabla`.`tituloc`, `traduccion_tabla`.`muestraentitulo` from `traduccion_tabla` $condition";
          return $this->Hydrating($query);
       }
       ...
       */
       ...
       {
          $query = "select $fAgregado($elemento) as result from `traduccion_tabla` $condition";
         ...
         $this->conn->setQuery($query);
     	 $list = array();
		    ...
		    $row = array();
     	    ...
     	    {
     	      $object = new  TraduccionTablaExtend();			  
     	      $object->Field = $row;
     	      //obtiene todas la clave/valores que sean iguales a los campos de la tabla TraduccionTabla
      ...
      public function Search($params = array(), $data= array() , $startRows = 0, $maxRows = 10)
    	{
               ...
               if($params["id"]) $cond[] = "`traduccion_tabla`.`id`=".$params["id"]; 

                if($params["tabla"]) $cond[] = "`traduccion_tabla`.`tabla`like '%".$params["tabla"]."%'"; 

                if($params["campo"]) $cond[] = "`traduccion_tabla`.`campo`like '%".$params["campo"]."%'"; 

                if($params["tipotrad"]) $cond[] = "`traduccion_tabla`.`tipotrad`=".$params["tipotrad"]; 

                if($params["tituloc"]) $cond[] = "`traduccion_tabla`.`tituloc`like '%".$params["tituloc"]."%'"; 

                if($params["muestraentitulo"]) $cond[] = "`traduccion_tabla`.`muestraentitulo`=".$params["muestraentitulo"]; 

                $q = $cond[0] ? "where ".implode(" and ", $cond) : "";

               $limit = $maxRows ? "limit $startRows, $maxRows" : "";

    	   return $this->Select("select SQL_CALC_FOUND_ROWS `traduccion_tabla`.`id`, `traduccion_tabla`.`tabla`, `traduccion_tabla`.`campo`, `traduccion_tabla`.`tipotrad`, `traduccion_tabla`.`tituloc`, `traduccion_tabla`.`muestraentitulo` from `traduccion_tabla` $q $limit");
    	}
      ...
      */	
	  ...
	  protected function parseParams(array $params, $tpQuery = "where"){ return $params[0] ? sprintf(" %s %s ", $tpQuery, implode(" and ", $params)) : ""; }
       ...
       {
         $this->Field["campo"] = $this->scapingData($Campo);
         $this->listModifiedField["campo"] = true;
       }

       public function setTipotrad($Tipotrad)
       {
         $this->Field["tipotrad"] = $this->scapingData($Tipotrad);
         $this->listModifiedField["tipotrad"] = true;
       }

       public function setTituloc($Tituloc)
       {
         $this->Field["tituloc"] = $this->scapingData($Tituloc);
         $this->listModifiedField["tituloc"] = true;
       }

       public function setMuestraentitulo($Muestraentitulo)
       {
         $this->Field["muestraentitulo"] = $this->scapingData($Muestraentitulo);
         $this->listModifiedField["muestraentitulo"] = true;
       }
	...
	*Version abreviada de getGenericField
       ...
       function getCampo()
       {
         return $this->Field["campo"];
       }

       function getTipotrad()
       {
         return $this->Field["tipotrad"];
       }

       function getTituloc()
       {
         return $this->Field["tituloc"];
       }

       function getMuestraentitulo()
       {
         return $this->Field["muestraentitulo"];
       }
