<?php
    class Pmodulos
    {
       ...
       public function Pmodulos()
       {
          $this->conn = JFactory::getDbo();          
          $this->Field = array(); 
          //aqui se van a guardar los formatos de los campos
          $this->formatFields = array('nombre'=>"`pmodulos`.`nombre` = '%s'", 'acceso'=>"`pmodulos`.`acceso` = '%s'", 'tipo'=>"`pmodulos`.`tipo` = %s");
          //listado de llaves primarias
          $this->listPk = array();
          //formato de las llaves primarias
          $this->listPkFormat = array("id"=>"`pmodulos`.`id` = %s");
          //listado de campos que contiene la tabla Pmodulos
          $this->listField = array("id"=>"id", "nombre"=>"nombre", "acceso"=>"acceso", "tipo"=>"tipo");
       ...
       {
          return $query = sprintf("insert into `pmodulos` (`pmodulos`.`nombre`, `pmodulos`.`acceso`, `pmodulos`.`tipo`) VALUES('%s', '%s', %s)", $this->Field["nombre"], $this->Field["acceso"], $this->Field["tipo"]);
       }
       ...
       {  
          //obtiene los campos a actualizar  
       	  ...
       	  return $query = "update `pmodulos` set ".implode(", ", $intersecUpdateField)." where ".implode(" and ", $intersecPk);
       }
       
       public function Delete()
       {
          $query =sprintf("delete from `pmodulos` where `pmodulos`.`id` = %s", $this->Field["id"]);
       ...
       {
          if($query == "")          
            $query = "select * from `pmodulos`";             
          
          return $this->Hydrating($query);          
       }
       
       /**
       * Este metodo devuelve un array con el objeto de tipo Pmodulos
       * dado un ID determinado
       */
       public function getPmodulosById($Id)
       {
         return $this->SelectConditional(sprintf("where `pmodulos`.`id` = %s", $Id));
       }
       ...
       */
       ...
       {
         $query = "select SQL_CALC_FOUND_ROWS `pmodulos`.`id`, `pmodulos`.`nombre`, `pmodulos`.`acceso`, `pmodulos`.`tipo` from `pmodulos` $condition";
          return $this->Hydrating($query);
       }
       ...
       */
       ...
       {
          $query = "select $fAgregado($elemento) as result from `pmodulos` $condition";
         ...
         $this->conn->setQuery($query);
     	 $list = array();
		    ...
		    $row = array();
     	    ...
     	    {
     	      $object = new  PmodulosExtend();			  
     	      $object->Field = $row;
     	      //obtiene todas la clave/valores que sean iguales a los campos de la tabla Pmodulos
      ...
      public function Search($params = array(), $data= array() , $startRows = 0, $maxRows = 10)
    	{
               ...
               if($params["id"]) $cond[] = "`pmodulos`.`id`=".$params["id"]; 

                if($params["nombre"]) $cond[] = "`pmodulos`.`nombre`like '%".$params["nombre"]."%'"; 

                if($params["acceso"]) $cond[] = "`pmodulos`.`acceso`like '%".$params["acceso"]."%'"; 

                if($params["tipo"]) $cond[] = "`pmodulos`.`tipo`=".$params["tipo"]; 

                $q = $cond[0] ? "where ".implode(" and ", $cond) : "";

               $limit = $maxRows ? "limit $startRows, $maxRows" : "";

    	   return $this->Select("select SQL_CALC_FOUND_ROWS `pmodulos`.`id`, `pmodulos`.`nombre`, `pmodulos`.`acceso`, `pmodulos`.`tipo` from `pmodulos` $q $limit");
    	}
      ...
      */	
	  ...
	  protected function parseParams(array $params, $tpQuery = "where"){ return $params[0] ? sprintf(" %s %s ", $tpQuery, implode(" and ", $params)) : ""; }
       ...
       {
         $this->Field["acceso"] = $this->scapingData($Acceso);
         $this->listModifiedField["acceso"] = true;
       }
