<?php
    class IdiomasT
    {
       ...
       public function IdiomasT()
       {
          $this->conn = JFactory::getDbo();          
          $this->Field = array(); 
          //aqui se van a guardar los formatos de los campos
          $this->formatFields = array('nombre'=>"`idiomas_t`.`nombre` = '%s'", 'abv'=>"`idiomas_t`.`abv` = '%s'", 'activo'=>"`idiomas_t`.`activo` = %s");
          //listado de llaves primarias
          $this->listPk = array();
          //formato de las llaves primarias
          $this->listPkFormat = array("id"=>"`idiomas_t`.`id` = %s");
          //listado de campos que contiene la tabla IdiomasT
          $this->listField = array("id"=>"id", "nombre"=>"nombre", "abv"=>"abv", "activo"=>"activo");
       ...
       {
          return $query = sprintf("insert into `idiomas_t` (`idiomas_t`.`nombre`, `idiomas_t`.`abv`, `idiomas_t`.`activo`) VALUES('%s', '%s', %s)", $this->Field["nombre"], $this->Field["abv"], $this->Field["activo"]);
       }
       ...
       {  
          //obtiene los campos a actualizar  
       	  ...
       	  return $query = "update `idiomas_t` set ".implode(", ", $intersecUpdateField)." where ".implode(" and ", $intersecPk);
       }
       
       public function Delete()
       {
          $query =sprintf("delete from `idiomas_t` where `idiomas_t`.`id` = %s", $this->Field["id"]);
       ...
       {
          if($query == "")          
            $query = "select * from `idiomas_t`";             
          
          return $this->Hydrating($query);          
       }
       
       /**
       * Este metodo devuelve un array con el objeto de tipo IdiomasT
       * dado un ID determinado
       */
       public function getIdiomasTById($Id)
       {
         return $this->SelectConditional(sprintf("where `idiomas_t`.`id` = %s", $Id));
       }
       ...
       */
       ...
       {
         $query = "select SQL_CALC_FOUND_ROWS `idiomas_t`.`id`, `idiomas_t`.`nombre`, `idiomas_t`.`abv`, `idiomas_t`.`activo` from `idiomas_t` $condition";
          return $this->Hydrating($query);
       }
       ...
       */
       ...
       {
          $query = "select $fAgregado($elemento) as result from `idiomas_t` $condition";
         ...
         $this->conn->setQuery($query);
     	 $list = array();
		    ...
		    $row = array();
     	    ...
     	    {
     	      $object = new  IdiomasTExtend();			  
     	      $object->Field = $row;
     	      //obtiene todas la clave/valores que sean iguales a los campos de la tabla IdiomasT
      ...
      public function Search($params = array(), $data= array() , $startRows = 0, $maxRows = 10)
    	{
               ...
               if($params["id"]) $cond[] = "`idiomas_t`.`id`=".$params["id"]; 

                if($params["nombre"]) $cond[] = "`idiomas_t`.`nombre`like '%".$params["nombre"]."%'"; 

                if($params["abv"]) $cond[] = "`idiomas_t`.`abv`like '%".$params["abv"]."%'"; 

                if($params["activo"]) $cond[] = "`idiomas_t`.`activo`=".$params["activo"]; 

                $q = $cond[0] ? "where ".implode(" and ", $cond) : "";

               $limit = $maxRows ? "limit $startRows, $maxRows" : "";

    	   return $this->Select("select SQL_CALC_FOUND_ROWS `idiomas_t`.`id`, `idiomas_t`.`nombre`, `idiomas_t`.`abv`, `idiomas_t`.`activo` from `idiomas_t` $q $limit");
    	}
      ...
      */	
	  ...
	  protected function parseParams(array $params, $tpQuery = "where"){ return $params[0] ? sprintf(" %s %s ", $tpQuery, implode(" and ", $params)) : ""; }
       ...
       {
         $this->Field["abv"] = $this->scapingData($Abv);
         $this->listModifiedField["abv"] = true;
       }

       public function setActivo($Activo)
       {
         $this->Field["activo"] = $this->scapingData($Activo);
         $this->listModifiedField["activo"] = true;
       }
