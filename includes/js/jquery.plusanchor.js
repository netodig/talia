    ...
    $.plusAnchor = function (el, options) {
        ...
        base.init = function () {
            ...
            base.$el.find('a[href^="#"]').click(function (e) {
                ...
                var iex = 0;
                var exout = false;
                var exhash = jQuery(this).attr('href');
                while (iex < base.options.exclude.length && !exout)
                {
                    if(base.options.exclude[iex++] == exhash)
                        exout = true;
                }
                if(exout) return;
                //fin agregdo por edgar
